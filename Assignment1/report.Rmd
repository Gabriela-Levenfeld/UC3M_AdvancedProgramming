...
For this case, we settle up $p = 2$. Hence, we are computing the euclidean distance and the result is the same as our previous computation.
...
-   **my_knn_c_tuningp**; Function which performance a search over a set of possible $p$ values provided by the user. And, after, with the value selected makes the prediction for X0. It also return the corresponding accuracy of the model.
...
For showing functionality of **my_knn_c_tuningp** function, two demos are execute.
